...
- Social copy is written only by the Codex agent flow documented in `docs/operating-guides/codex-content-automation.md`.
...
5. Use the `social-media-art-director` skill with the exported brief facts to generate 2-3 real image files through Magnific MCP using Google Nano Banana 2. Natural prompts like "create three Facebook image options for this planned post" should trigger it from `.agents/skills/social-media-art-director`.
6. Download generated files under `outputs/graphics/planning/uploads/planned-item-<id>/`.
...
- Use `social_media_art_director_contracts(brief, count=3)` from `marketing_os.services.skill_adapters`.
